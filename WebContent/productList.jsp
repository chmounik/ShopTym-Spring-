...
<body>
	...
	<%!
		...
		ArrayList<Product> productsFromSession;
	%>

	<%
		if (session.getAttribute("productsFromSession") != null) {
			//out.write("Session working");
			productsFromSession = (ArrayList<Product>) session.getAttribute("productsFromSession");
		}
	...
	<div class="login">
		<div class="wrap">
			<div class="rsidebar span_1_of_left">
				<form name="filterProductsForm" action="${contextPath}/product/productList/filterProducts" method="post">
					<input type="hidden" name="productCategoryForFilter"
						value="${productCategoryFromSession}">
			...
			<form action="${contextPath}/search/searchProducts">
			<div class="search">
				<input type="text" name="key"></input> 
<!-- 				<input type="radio" -->
<!-- 					name="type" value="PRODUCTNAME">Product Name <input -->
<!-- 					type="radio" name="type" value="BRANDNAME">Product Brand <select -->
<!-- 					name="PRODUCTTYPE"> -->
 ...
 --%>
			<div class="cont span_2_of_3">
				<div class="mens-toolbar">
					<div class="sort">
						<div class="sort-by">
							<form name="productSortForm" action="${contextPath}/product/productList/sortProducts" method="post">
								<label>Sort By</label>
									<select name="productSortBy" onchange="this.form.submit()">
										<option value="">Default</option>
										<option value="A2Z" <c:if test="${productSortBy == 'A2Z' }">selected</c:if> >A to Z</option>
										<option value="Z2A" <c:if test="${productSortBy == 'Z2A' }">selected</c:if> >Z to A</option>
										<option value="HIGH" <c:if test="${productSortBy == 'HIGH' }">selected</c:if> >Price : High to Low</option>
										<option value="LOW" <c:if test="${productSortBy == 'LOW' }">selected</c:if> >Price : Low to High</option>
				...
				<c:forEach items="${productsFromSession}" var="product">
					<div class="box1">

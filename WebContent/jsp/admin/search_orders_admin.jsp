...
<body>
	...
	<table id="customers">
		...
		<tbody>
			<tr>
			<form name="searchOrders" method="post" action="${contextPath}/admin/searchOrders">
				<td>
				...
				<td colspan="2"><input type="submit" value="Search Orders"></td>
			</form>
			</tr>
			<c:forEach var="order" items="${orders}">
				<tr>
					<td>${ order.getOrderID() }</td>
					<td>${ order.isTransactionSuccessful() }</td>
					<td>${ order.getUserEmail() }</td>
					<td>${ order.getContactNo() }</td>
					<td>${ order.getOrderDate() }</td>
					<td><a href = "${contextPath}/admin/getOrderDetails?orderID=${order.getOrderID()}">View details</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>

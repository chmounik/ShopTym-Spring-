...
<body>
	...
	<br/>
	<%!	ArrayList<User> sellerRequests ;
	%>
	<%
		if (session != null && session.getAttribute("sellerRequests") != null) {
			sellerRequests = (ArrayList<User>) session.getAttribute("sellerRequests");
		}
		...
		if (sellerRequests.size() == 0) {
			out.write("<h4>No pending seller requests found</h4>");
		}
	%>
	<table id="customers">
		<thead>
		...
		<tbody>
			<c:forEach var="request" items="${sellerRequests}">
				<tr>
					<td>${ request.getEmail() }</td>
					<td>${ request.getFirstName() }</td>
					<td>${ request.getLastName() }</td>
					<td>${ request.getContactNo() }</td>
					<td><a href = "${contextPath}/admin/approveSeller?email=${ request.getEmail() }">Approve</a></td>
					<td><a href = "${contextPath}/admin/rejectSeller?email=${ request.getEmail() }">Reject</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>

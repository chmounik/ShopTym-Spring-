...
<body>
	...
	<h4><a href="${contextPath}/admin/viewAllOrders"> Go back to 'All Orders' page </a></h4>

	<%!	ArrayList<ViewOrder_Admin> viewOrderList ;
	%>
	<%
		if (session != null && session.getAttribute("viewOrderList") != null) {
			viewOrderList = (ArrayList<ViewOrder_Admin>) session.getAttribute("viewOrderList");
		}
	%>
	<table id="customers">
		<thead>
			<tr>
				<th>Shopping Cart Product ID</th>
				<th>Product Name</th>
				<th>Seller Email</th>
				<th>Product Type</th>
		...
		<tbody>
			<c:forEach var="viewOrder" items="${viewOrderList}">
				<tr>
					<td>${ viewOrder.getShoppingCartProductID() }</td>
					<td>${ viewOrder.getProductName() }</td>
					<td>${ viewOrder.getSellerEmail() }</td>
					<td>${ viewOrder.getProductType() }</td>
					<td>${ viewOrder.getProductCategoryName() }</td>

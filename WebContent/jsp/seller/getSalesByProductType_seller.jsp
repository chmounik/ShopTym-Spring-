...
<body>
	...
	<br/>
	<table>
		<form action="${contextPath}/seller/getSalesByProductTypeSeller" method="GET">
			<tr>
				<td><input type="text" name="productType"/></td>
			</tr>
			<tr>
				<td><input type='submit' value='Search Product Type' /></td>
			</tr>
		</form>
	</table>
	...
	<table id="customers">
		...
		<tbody>
			<c:forEach var="saleForProductTypeSeller" items="${salesByProductTypeSeller}">
				<tr>
					<td>${ saleForProductTypeSeller.getProductType() }</td>
					<td>${ saleForProductTypeSeller.getSales() }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<script>
		var ba = [ <%=session.getAttribute("graphData").toString()%> ];
		var buyerData = {
			labels : [ <%=session.getAttribute("graphLabels").toString()%> ],
			datasets : [ {
